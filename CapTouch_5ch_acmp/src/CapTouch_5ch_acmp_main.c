...
/* [DEBUG] Only to monitor the registers in the memory.
LPC_IOCON_TypeDef * volatile gLpcIoconBase = LPC_IOCON;
LPC_SWM_TypeDef * volatile gLpcSwmBase = LPC_SWM;
*/
...
/*
* Setup the ACMP for CapTouch application use case.
* PIO0_16 is used the ACMP_I4 for CapTouch application, to capture the crossing trigger event.
*/
void App_CapTouch_InitAcmp(void)
{
    /* Enable power & clock for ACMP module. */
    LPC_SYSCON->PDRUNCFG &= ~(ACMP_PD);
    Enable_Periph_Clock(CLK_ACMP);
    Do_Periph_Reset(RESET_ACMP);

    /* Setup pin for ACMP module. */
    EnableFixedPinFunc(ACMP_I4);
    LPC_IOCON->PIO0_16 = IOCON_PIO_MODE(0) /* | IOCON_PIO_OD_MASK */ | IOCON_PIO_RESERVED; /* ACMP_I4 for CapTouch. */

    /* Configure the ACMP. */
    LPC_CMP->CTRL = (_20mV << HYS)
                  | (0 << INTENA) /* Disable interrupt, but the interrupt is enabled by CAPT. */
                  | (V_LADDER_OUT << COMP_VM_SEL) /* Use internal DAC as the Minus input. */
                  | (ACOMP_IN4 << COMP_VP_SEL) /* Use ACMP_I4 as the Plus input. */
                  ;
    LPC_CMP->LAD = (1U << LADEN)
                 | (0U << LADREF) /* Use internal VDD as the reference voltage. */
                 | (16 << LADSEL) /* output of internal DAC. 0 - 31 */
                 ;
}
...
void App_CapTouch_Init(void)
{
    ...
    /* Enable the ACMP for CapTouch application. */
    App_CapTouch_InitAcmp();
     ...
     */
    while (0U != (INCHANGE & LPC_CAPT->CTRL)) /* Waiting while the LPC_CAPT is in change. */
    {}
    LPC_CAPT->CTRL = POLLMODE_INACTIVE /* Would launch the scan later. */
                   | TYPE_TRIGGER_ACMP /* TYPE_TRIGGER_ACMP */
	               | WAIT  /* NO wait, since no continuous mode is used. */
                   | (((15000000UL/2500000UL)-1U) << FDIV) /* Divider for FCLK from clock source. */
    ...
    LPC_CAPT->POLL_TCNT = (0U  << TCNT) /* No hardware judgment for YES/NO touch. Software would check the sensing value after PollDone event. */
                        ...
                        | (10U << POLL)  /* x4096 FCLK between poll round in continuous scan mode. */
